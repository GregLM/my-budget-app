        ...
        <Drawer.Content className="bg-background flex flex-col rounded-t-[40px] fixed bottom-0 left-0 right-0 z-50 outline-none max-w-md mx-auto h-[96vh] border-t border-border shadow-2xl">
          <div className="p-5 flex-1 overflow-y-auto">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
              ...
              <div className="flex bg-muted p-1 rounded-xl h-12 shrink-0">
                <button type="button" onClick={() => setValue('type', 'expense')} className={`flex-1 rounded-xl text-xs font-black uppercase transition-all ${watch('type') === 'expense' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground'}`}>Dépense</button>
                <button type="button" onClick={() => setValue('type', 'income')} className={`flex-1 rounded-xl text-xs font-black uppercase transition-all ${watch('type') === 'income' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground'}`}>Revenu</button>
              </div>

              {/* Montant avec clavier numérique decimal */}
              ...
              {/* Grid 2 colonnes pour Libellé et Date (gain de place vertical) */}
              <div className="grid grid-cols-2 gap-3">
                <input type="text" {...register('description', { required: true })} placeholder="Libellé" className="bg-muted p-5 rounded-2xl font-bold text-sm outline-none text-foreground w-full" />
                <input type="date" {...register('date', { required: true })} className="bg-muted p-5 rounded-2xl text-xs font-bold outline-none text-foreground w-full" />
              </div>

              <div className="flex items-center justify-between p-5 bg-muted rounded-2xl">
                <span className="text-[10px] font-black uppercase opacity-40 text-foreground">Mouvement récurrent</span>
                <input type="checkbox" {...register('isFixed')} className="w-6 h-6 accent-blue-600 rounded-lg" />
              </div>

              {/* Grille de catégories compacte (3 colonnes) */}
              ...
              {/* Bouton de validation bien espacé du bas pour iPhone */}
